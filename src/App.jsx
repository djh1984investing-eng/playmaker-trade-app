...
const exportJournalCSV = () => {
  ...
  const showDashboardPanels = tab === "trade";
...
function Journal({ journal, saveTrade, editTrade, exportJournalCSV, form, set, editingId, handleImageUpload }) {
  ...
  const totalMaxMove = closed.reduce((sum, j) => sum + n(j.maxMove), 0);
  const totalDrawdown = closed.reduce((sum, j) => sum + n(j.maxDrawdown), 0);
  const winRate = closed.length ? Math.round((wins / closed.length) * 100) : 0;
  const lossRate = closed.length ? Math.round((losses / closed.length) * 100) : 0;
  const avgMove = closed.length ? totalMaxMove / closed.length : 0;
  const avgDrawdown = closed.length ? totalDrawdown / closed.length : 0;
  const bestMove = closed.length ? Math.max(...closed.map((j) => n(j.maxMove))) : 0;
  const worstDrawdown = closed.length ? Math.max(...closed.map((j) => n(j.maxDrawdown))) : 0;
    ...
    <div className="mt-6 grid gap-5 lg:grid-cols-[420px_1fr]">
      ...
      <Card>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-4">
          <Small label="Closed Trades" value={closed.length} />
          <Small label="Win Rate" value={`${winRate}%`} />
          <Small label="Loss Rate" value={`${lossRate}%`} />
          <Small label="Open / Unfilled" value={unfilled} />
          <Small label="Wins" value={wins} />
          <Small label="Losses" value={losses} />
          <Small label="BE" value={be} />
          <Small label="Total P/L" value={`$${fmt(totalPL)}`} />
          <Small label="Average P/L" value={`$${fmt(avgPL)}`} />
          <Small label="Average Max Move" value={`${fmt(avgMove)} pts`} />
          <Small label="Best Max Move" value={`${fmt(bestMove)} pts`} />
          <Small label="Average Drawdown" value={`${fmt(avgDrawdown)} pts`} />
        </div>
              ...
              {journal.map((j) => (
                <React.Fragment key={j.id}>
                  <tr className="border-t border-zinc-800">
                    ...
                    <td className="font-black text-[#ffcc19]">{j.entry ? fmtPrice(j.entry) : "--"}</td>
                    <td>{j.direction}</td>
