...
Code: ${error?.code || ""}`
  ...
  const saveTradeMemory = async (item) => {
    // AI learning database is owner-only. Customer journals still save privately
    // in trade_journal and calculate each customer's own stats from their own rows.
    if (!ownerMode || !user || !item || isOpenOrder(item)) return;
